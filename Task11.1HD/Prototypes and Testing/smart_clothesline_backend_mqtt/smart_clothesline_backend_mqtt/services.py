...
def publish_mqtt_command(command: str, source: str = "backend", reason: Optional[str] = None) -> Dict[str, Any]:
    ...
    with runtime.lock:  # Lock the door so shared backend data can be updated safely
        if cmd == "SET_ACTIVE_TRUE":
            # Write on the backend's shared "whiteboard" that laundry is active
            runtime.laundry_active = True
        elif cmd == "SET_ACTIVE_FALSE":
            # Write on the backend's shared "whiteboard" that laundry is not active
    ...
    with _mqtt_lock:           # 🚪 LOCK the door
        client = _mqtt_client  # Use the MQTT client (safely)

        if client is None or not client.is_connected():
        
            #` Check if the MQTT client is missing or not connected

            with runtime.lock:
                # Save the command that could not be sent
                runtime.pending_command = cmd

                # Record the command that was attempted
                runtime.last_command_published = cmd

                # Record where the command came from, such as dashboard or API
                runtime.last_command_source = source

                # Record the time when the command was attempted
                runtime.last_command_time = iso_now()

                # Store the MQTT error message
                runtime.mqtt_last_error = "MQTT not connected; command could not be published"

            # Add a failed command event to the event history/log
            add_event(
            ...
            # Return an error response because the command was not published
            return {
                "ok": False,
        ...
        info = client.publish(Config.MQTT_COMMAND_TOPIC, payload_text, qos=0, retain=False)# Send the message
...
def build_fallback_decision(payload: Optional[Dict[str, Any]] = None, telemetry_is_live: bool = True) -> Dict[str, Any]:
    ...
    if rain_status == "CONFIRMED":
        should_move = state not in {"SAFE", "MOVING", "LINE_HUNT", "TURNING_AROUND"} and target not in {"SAFE"}
        return {
            "enabled": True,
            "active": should_move,
    ...
    weather = fetch_weather(force=False)
    weather_bad = bool(weather.get("rain_likely"))

    should_move = (
        weather_bad
        and state not in {"SAFE", "MOVING", "LINE_HUNT", "TURNING_AROUND"}
        and target not in {"SAFE"}
    )

    return {
        "enabled": True,
        "active": should_move,
        "reason": "RAIN_SENSOR_FAULT_WEATHER_BAD" if weather_bad else "RAIN_SENSOR_FAULT_WEATHER_OK",
        "recommended_command": "MOVE_TO_SAFE" if should_move else "NONE",
        "rain_sensor": rain_eval,
