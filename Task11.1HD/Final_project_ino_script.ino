/*
  ...
  Lock-Based Hunt + Explicit Zone Codes + Rain + MQTT + Stable Zone Verification + Forward Commit 250ms + Minimal Light Shade + 60s Search Timeout
...
// For trend-aware light/shade detection.
float previousLeftLux = -1;
float previousRightLux = -1;
bool leftShadeChecking = false;
bool rightShadeChecking = false;
unsigned long leftShadeCheckStartTime = 0;
unsigned long rightShadeCheckStartTime = 0;
float leftShadeReferenceLux = -1;
float rightShadeReferenceLux = -1;
unsigned long sunLockUntil = 0;
bool lightShadeFault = false;
...
// Minimal upgrades:
// 1) Search timeout: if the robot cannot reach the target within 60 seconds, stop safely.
// 2) Trend-aware light handling: increasing brightness is useful; a sharp drop means shade/cover.
const unsigned long SEARCH_TIMEOUT_MS = 60000;
const unsigned long SUN_SHADE_LOCK_MS = 10000;
const unsigned long LIGHT_SHADE_CONFIRM_MS = 3000;
const float LIGHT_MIN_VALID_LUX = 2.0;
const float LIGHT_SHADE_DROP_RATIO = 0.45;      // current reading < 45% of previous/reference
const float LIGHT_SHADE_COMPARE_RATIO = 0.50;   // shaded sensor is less than half of other sensor
...
// Target movement/search timeout. This is deliberately long so it does not affect normal demo movement.
unsigned long targetSearchStartTime = 0;
bool searchTimeoutFault = false;
...
void publishTelemetryToMQTT() {
  ...
  doc["search_timeout_fault"] = searchTimeoutFault;
  doc["light_shade_fault"] = lightShadeFault;
...
void handleMQTTCommand(String command) {
  ...
  if (command == "MOVE_TO_SAFE") {
    // Safety command: accept it even during movement so backend rain fallback can send the robot safe.
    setEvent("MQTT_CMD_MOVE_TO_SAFE_ACCEPTED");
    publishStatusMessage("CMD_MOVE_SAFE_ACCEPTED");
    goToTarget(TARGET_SAFE);
    return;
  }
...
void goToTarget(TargetZone requestedTarget) {
  ...
  // Start a long safety timer for the whole mission.
  // This should not interrupt a normal run; it only stops the robot if it searches too long.
  targetSearchStartTime = millis();
  searchTimeoutFault = false;
...
void checkSearchTimeout() {
  ...
  if (targetSearchStartTime == 0) return;
  if (!isMovementCriticalState()) return;

  if (millis() - targetSearchStartTime >= SEARCH_TIMEOUT_MS) {
    stopMotors();
    state = STOPPED;
    target = TARGET_NONE;
    targetSearchStartTime = 0;
    searchTimeoutFault = true;
    setEvent("SEARCH_TIMEOUT_STOPPED");
    publishStatusMessage("SEARCH_TIMEOUT_STOPPED");
  }
}
...
void updateSunTracking() {
  ...
  // Lockout is only used for a real shade/cover condition, not for normal brightness difference.
  if (now < sunLockUntil) {
    stopMotors();
    return;
  }
  ...
  bool havePrevious = previousLeftLux >= 0 && previousRightLux >= 0;

  // Trend-aware shade detection:
  // A large brightness difference alone is NOT a fault. That is normal sun direction.
  // A sharp drop from the sensor's own previous/reference value means shade/cover.
  if (havePrevious && !leftShadeChecking &&
      previousLeftLux > LIGHT_MIN_VALID_LUX &&
      leftLux < previousLeftLux * LIGHT_SHADE_DROP_RATIO &&
      leftLux < rightLux * LIGHT_SHADE_COMPARE_RATIO) {
    leftShadeChecking = true;
    leftShadeCheckStartTime = now;
    leftShadeReferenceLux = previousLeftLux;
    setEvent("LEFT_LIGHT_SHADE_CHECKING");
  }

  if (havePrevious && !rightShadeChecking &&
      previousRightLux > LIGHT_MIN_VALID_LUX &&
      rightLux < previousRightLux * LIGHT_SHADE_DROP_RATIO &&
      rightLux < leftLux * LIGHT_SHADE_COMPARE_RATIO) {
    rightShadeChecking = true;
    rightShadeCheckStartTime = now;
    rightShadeReferenceLux = previousRightLux;
    setEvent("RIGHT_LIGHT_SHADE_CHECKING");
  }

  if (leftShadeChecking) {
    bool stillLeftShaded =
      leftLux < leftShadeReferenceLux * LIGHT_SHADE_DROP_RATIO &&
      leftLux < rightLux * LIGHT_SHADE_COMPARE_RATIO;

    if (stillLeftShaded && now - leftShadeCheckStartTime >= LIGHT_SHADE_CONFIRM_MS) {
      lightShadeFault = true;
      sunDirection = 0;
      sunLockUntil = now + SUN_SHADE_LOCK_MS;
      setEvent("LEFT_LIGHT_SHADED_LOCKED");
      stopMotors();
      return;
    }

    if (!stillLeftShaded) {
      leftShadeChecking = false;
      leftShadeReferenceLux = -1;
      lightShadeFault = false;
    }
  }

  if (rightShadeChecking) {
    bool stillRightShaded =
      rightLux < rightShadeReferenceLux * LIGHT_SHADE_DROP_RATIO &&
      rightLux < leftLux * LIGHT_SHADE_COMPARE_RATIO;

    if (stillRightShaded && now - rightShadeCheckStartTime >= LIGHT_SHADE_CONFIRM_MS) {
      lightShadeFault = true;
      sunDirection = 0;
      sunLockUntil = now + SUN_SHADE_LOCK_MS;
      setEvent("RIGHT_LIGHT_SHADED_LOCKED");
      stopMotors();
      return;
    }

    if (!stillRightShaded) {
      rightShadeChecking = false;
      rightShadeReferenceLux = -1;
      lightShadeFault = false;
    }
  }
  ...
  if (diff > -LIGHT_DEADBAND && diff < LIGHT_DEADBAND) {
    ...
    setEvent("SUN_ROTATE_LEFT_BRIGHTER");
  }

  else {
    sunDirection = 1;
    sunRotateUntil = now + SUN_ROTATE_STEP_MS;
    setEvent("SUN_ROTATE_RIGHT_BRIGHTER");
  }

  previousLeftLux = leftLux;
  previousRightLux = rightLux;
}
...
void printStatus() {
  ...
  Serial.print(searchTimeoutFault ? "YES" : "NO");

  Serial.print(" | LightShade: ");
  Serial.print(lightShadeFault ? "YES" : "NO");
